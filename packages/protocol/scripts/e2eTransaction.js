...
} = require("../deployments/matic/ZeroController.json");
const {
  abi: BTCVaultAbi,
  address: BTCVaultAddress,
} = require("../deployments/matic/BTCVault");
...
const keeperCallback = async (msg) => {
  ...
  try {
    ...
    console.log(
      `(TransferRequest) Deposit ${utils.formatUnits(tr.amount, 8)} BTC to ${
        mint.gatewayAddress
      }`
    );
    ...
    const loanTx = await tr.loan(
      new ethers.Wallet(process.env.UNDERWRITER_WALLET).connect(signer.provider)
    );
    console.log("loaned!");
    console.log(loanTx);
    ...
    const tx = await tr.repay(
      new ethers.Wallet(process.env.UNDERWRITER_WALLET).connect(
        signer.provider
      ),
      { gasLimit: 500e3 }
    );
...
const makeUser = async () => {
  const user = sdk.createZeroUser(
    await sdk.createZeroConnection(
...
const main = async () => {
  ...
  const underwriterImpl = new Contract(
    underwriterAddress,
    ControllerAbi,
    new ethers.Wallet(process.env.UNDERWRITER_WALLET).connect(signer.provider)
  );
  ...
  transferRequest = new TransferRequest({
    ...
    pNonce:
      "0x46cbcf365ecad2171742b1adeecb4b3d74eb0fddb8988b690117bf550a9b19e9",
  ...
  console.log(
    `(TransferRequest) Deposit ${utils.formatUnits(
      transferRequest.amount,
      8
    )} BTC to ${gatewayAddress}`
  );
