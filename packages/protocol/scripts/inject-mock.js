const {
  ForkStateManager,
} = require("hardhat/internal/hardhat-network/provider/fork/ForkStateManager");
const { ethers } = require("ethers");

const { getContractCode } = ForkStateManager.prototype;

const toString = (v) =>
  ethers.utils.getAddress(v.toString("hex")).toLowerCase();
  ...
  value: function (address) {
    ...
    if (exports.overrides[address]) {
      return Promise.resolve(
        Buffer.from(exports.overrides[address].substr(2), "hex")
      );
    }
