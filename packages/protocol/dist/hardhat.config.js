...
function getRemappings() {
    return (0, fs_1.readFileSync)("remappings.txt", "utf8")
        .split("\n")
        .filter(Boolean)
                    ...
                    switch (_a.label) {
                        case 0:
                            ...
                            return [4 /*yield*/, hre.network.provider.send("hardhat_setCode", [
                                    hre.ethers.utils.getAddress(common_1.FIXTURES[process.env.CHAIN].btcGateway),
...
var accounts = [
    wallet || (0, crypto_1.randomBytes)(32).toString("hex"),
    process.env.UNDERWRITER || (0, crypto_1.randomBytes)(32).toString("hex"),
];
        ...
        eachLine: function (hre) { return ({
            transform: function (line) {
                if (line.match(/^\s*import /i)) {
                    getRemappings().forEach(function (_a) {
                        var find = _a[0], replace = _a[1];
                        if (line.match(find)) {
