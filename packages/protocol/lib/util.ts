...
export const getProvider = (transferRequest) => {
  const chain = Object.entries(CONTROLLER_DEPLOYMENTS).find(
    ([k, v]) => transferRequest.contractAddress === v
  );
  const chain_key = chain[0];
  return RENVM_PROVIDERS[chain_key](
    new ethers.providers.JsonRpcProvider(RPC_ENDPOINTS[chain_key]),
    "mainnet"
  );
};
