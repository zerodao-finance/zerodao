...
export class ValidatorSet implements IValidatorSet {
  ...
  static fromBuffer(_vBuffer: Buffer) {
    ...
    var { validators, proposer } = protocol.ValidatorSet.toObject(
      validatorMessage,
      {
        longs: String,
        enums: String,
  ...
  add(validator: Validator) {
    validator.priority =
      (_.sumBy(this.validators, (v: Validator) => v.power) + validator.power) *
      (this as any).constructor.INIT_PENALTY;
  ...
  // returns Buffer
  toBuffer = function () {
    let _message = this.protocol.Validator.create({
