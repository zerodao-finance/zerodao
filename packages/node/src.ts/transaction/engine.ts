  ...
  async runTransaction(tx: TX) {
    ...
    if (tx.type == 0) {
      try {
        ...
        const fromBalance =
          Number(oldFromAccount.unStakedBalance) - Number(tx.amount);
        ...
        const toBalance =
          Number(oldToAccount.unStakedBalance) + Number(tx.amount);
      ...
      try {
        ...
        const fromBalance =
          Number(oldFromAccount.unStakedBalance) - Number(tx.amount);
        const fromStakedBalance =
          Number(oldFromAccount.stakedBalance) + Number(tx.amount);
        const newFromAccount: Account = {
          ...oldFromAccount,
          unStakedBalance: fromBalance,
