...
function ValidateAndComplete(doc: GenesisDoc) {
	if ( doc.ChainID == "" ) throw new Error("genesis doc must include non-empty chain_id")
	if ( doc.InitialHeight < 0 ) throw new Error(`initial_height cannot be negative (got ${doc.InitialHeight})`)
