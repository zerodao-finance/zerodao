...
const isZcashAddress = (hex) =>
  ((hex) =>
    ...
    buffer_1.Buffer.isBuffer(hex) ? "0x" + hex.toString("hex") : hex
  );
const cache = {};
    ...
    for (let i = 0; i < (await this.redis.llen("/zero/pending")); i++) {
      try {
        ...
        if (utxos && utxos.length) {
          ...
          this.logger.info(
            util_1.default.inspect(utxos, { colors: true, depth: 15 })
          );
          // const redis = require('ioredis')(process.env.REDIS_URI);
          const contractAddress = (0, address_1.getAddress)(
          ...
          if (VAULT_DEPLOYMENTS[contractAddress]) {
            await this.redis.lpush(
              "/zero/dispatch",
              JSON.stringify(
                new request_1.TransferRequestV2(
                  transferRequest
