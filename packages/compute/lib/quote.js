...
exports.makeCompute =
  exports.makeQuoter =
  exports.getChainNameFixture =
  exports.applyRatio =
  exports.keeperReward =
    void 0;
...
function makeQuoter(CHAIN = "1", provider) {
  ...
  const renCrv = new contracts_1.Contract(
    common_1.FIXTURES[(0, exports.getChainNameFixture)(chain.name)][
      "Curve_Ren"
    ],
    [
  ...
  const quoter = new contracts_1.Contract(
  ...
  const getRenZECETHQuote = async (direction, amount) => {
    const RENZEC = new UNISWAP.Token(
      ...
      common_1.FIXTURES.ETHEREUM.renZEC,
      8
    );
  ...
  const getUSDTQuote = async (direction, amount) => {
    const tricrypto = new contracts_1.Contract(
      common_1.FIXTURES[(0, exports.getChainNameFixture)(chain.name)][
        "tricrypto"
      ],
  ...
  const getAVAXQuote = async (direction, amount) => {
    const WBTC = new JOE.Token(
      ...
      common_1.FIXTURES.AVALANCHE.WBTC,
      8
    );
    ...
    if (direction) {
      ...
      return await getWbtcQuote(
        false,
        (0, units_1.parseUnits)(trade.outputAmount.toExact(), 8)
      );
    }
  };
  ...
  const getUsdcQuoteAVAX = async (direction, amount) => {
    ...
    const aTricrypto = new contracts_1.Contract(
      "0xB755B949C126C04e0348DD881a5cF55d424742B2",
      ["function get_dy(uint256, uint256, uint256) view returns (uint256)"],
      chain.provider
    );
    const crvUSD = new contracts_1.Contract(
  ...
  const getUSDCNativeQuote = async (direction, amount) => {
    const usdcpool = new contracts_1.Contract(
      common_1.FIXTURES.AVALANCHE.USDC_POOL,
      ["function get_dy(int128, int128, uint256) view returns (uint256)"],
      chain.provider
    );
    if (direction) {
  ...
  const getRenBTCForOneETHPrice = async () => {
    if (chain.name === "AVALANCHE") {
      ...
      const renBTC = new UNISWAP.Token(
        UNISWAP.ChainId.MAINNET,
        common_1.FIXTURES.ETHEREUM.renBTC,
        8
      );
      const pair = await UNISWAP.Fetcher.fetchPairData(
        renBTC,
  ...
  const getRenZECUSDTQuote = async (direction, amount) => {
    if (direction) {
      ...
      return await wethToTokenQuote(
        direction,
        common_1.FIXTURES[chain.name].USDT,
        _amount
      );
    } else {
      const _amount = await wethToTokenQuote(
  ...
  const renBTCToETH = async (amount) => {
    if (chain.name === "AVALANCHE") {
      ...
      path.splice(
        2,
        (0, exports.getChainNameFixture)(chain.name) !== "MATIC" ? 2 : 0
      );
      const wbtcOut = await getWbtcQuote(true, amount);
      const quote = await quoter.quoteExactInput(
        (0, solidity_1.pack)(
          ["address", "uint24", "address"].concat(
            (0, exports.getChainNameFixture)(chain.name) === "MATIC"
              ? ["uint24", "address"]
              : []
          ),
          path
...
function makeCompute(CHAIN = "1") {
  ...
  const renJS = new ren_1.default("mainnet").withChains(
    bitcoin,
    zcash,
    arbitrum,
    avalanche,
  ...
  const computeTransferOutput = async ({ module, amount, primaryToken }) => {
    ...
    switch (module) {
      case common_1.FIXTURES[
        (0, exports.getChainNameFixture)(quotes.chain.name)
      ].USDC:
  ...
  const applyFee = async (amountIn, zeroFee, renVmFee, primaryToken) => {
    ...
    const gasFee = await computeGasFee(
      GAS_COST.add(exports.keeperReward.div(gasPrice)),
      gasPrice,
      primaryToken
    );
    ...
    try {
      ...
      renOutput = bignumber_1.BigNumber.from(
