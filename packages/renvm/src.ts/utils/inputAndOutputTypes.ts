  ...
  if (await toChain.isLockAsset(asset)) {
    ...
    if (!isContractChain(fromChain)) {
      throw ErrorWithCode.updateError(
        new Error(`Cannot burn from non-contract chain ${fromChain.chain}.`),
        RenJSError.PARAMETER_ERROR
      );
    }
    if (!(await fromChain.isMintAsset(asset))) {
      throw ErrorWithCode.updateError(
        new Error(`Asset '${asset}' is not supported on ${fromChain.chain}.`),
        RenJSError.PARAMETER_ERROR
      );
    }
    return {
    ...
    if (!(await toChain.isMintAsset(asset))) {
      throw ErrorWithCode.updateError(
        new Error(`Asset '${asset}' is not supported on ${toChain.chain}.`),
        RenJSError.PARAMETER_ERROR
      );
    }
    return {
