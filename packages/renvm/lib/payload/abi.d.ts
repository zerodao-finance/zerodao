...
export interface EthArg<name extends string = string, type extends EthType = EthType, valueType = any> {
    name: name;
    type: type;
    value: valueType;
    components?: AbiInput[];
...
export declare type EthArgs = EthArg[];
export declare const payloadToABI: (methodName: string, payload: Array<{
    type: string;
    name: string;
    value: unknown;
}> | undefined) => AbiItem[];
export declare const payloadToMintABI: (methodName: string, payload: Array<{
    type: string;
    name: string;
    value: unknown;
}> | undefined) => AbiItem;
...
export declare enum OverrideContractCallError {
    OverrideArrayLengthError = "Contract call override must be same length as contract calls array."
}
export declare const overrideContractCall: (contractCall: ContractCall, override: ContractCallOverride) => ContractCall;
export declare type ContractCallOverride = Partial<Omit<ContractCall, "contractParams"> & {
    contractParams: Array<Partial<EthArg>>;
}>;
export declare const overrideContractCalls: (contractCalls: ContractCall[], override: ContractCallOverride | ContractCallOverride[]) => ContractCall[];
export {};
