        ...
        JSON.stringify(
          {
            ...
            chainId: request_1.Request.addressToChainId(
...
const runKeeper = () => {
  (async () => {
    ...
    const signer = new wallet_1.Wallet(process.env.WALLET).connect(
      new providers_1.InfuraProvider("mainnet", chains_1.CHAINS[1].rpcUrl)
    );
    ...
    peer.on("zero:request:2.1.0", async (data) => {
      await handleEvent(
        (0, util_1.serializeToJSON)((0, request_1.deserialize)(data))
      );
    });
