...
class Dispatcher {
  constructor({ redis, gasLimit, logger, signer }) {
    ...
    this.providers = Object.entries(this.constructor.RPC_ENDPOINTS).reduce(
      (r, [key, value]) => {
    ...
    this.signers = Object.entries(this.constructor.RPC_ENDPOINTS).reduce(
      (r, [key, value]) => {
        r[key] = (
          NO_FLASHBOTS[key]
            ? (v) => v
            : (v) =>
                (0, ethers_flashbots_1.makePrivateSigner)({
                  signer: v,
  ...
  async runLoop() {
    ...
    while (true) {
      ...
      try {
        ...
        this.logger.info(
          util_1.default.inspect(tx, { colors: true, depth: 15 })
        );
        try {
          ...
          if (
            ...
            if (
              typeof hadError === "string" &&
              hadError.includes("transfer amount exceeds balance")
            ) {
              await this.redis.lrem("/zero/dispatch", txSerialized, 1);
            } else if (typeof hadError === "object") {
              if (
                JSON.stringify(hadError).includes(
          ...
          if (tx.chainId === 1)
            dispatched.wait = (0, wait_1.makeFlashbotsWait)(
              dispatched,
              this.logger
            ); // poll flashbots protect
