...
function getDomainStructure(request) {
  return Number(request.getChainId()) == 137 &&
    (0, address_1.getAddress)(request.asset) ===
      (0, address_1.getAddress)(common_1.FIXTURES.MATIC.USDC)
    ? [
        {
          name: "name",
          type: "string",
...
function isAsset(assetName, address) {
  return Boolean(
    Object.keys(common_1.FIXTURES).find(
      (network) =>
        (0, address_1.getAddress)(
          common_1.FIXTURES[network][assetName] ||
            constants_1.AddressZero.substr(0, 41) + "1"
  ...
  if (isUSDC(request.asset)) {
    if (chainId === 137) {
      return {
        ...
        salt: (0, bytes_1.hexZeroPad)(
          bignumber_1.BigNumber.from(String(chainId) || "1").toHexString(),
          32
        ),
      };
    }
...
function getRenAsset(request) {
  ...
  const address =
    common_1.FIXTURES[(0, common_1.toFixtureName)(request.getChainId())][
      (0, common_1.getRenAssetName)(request)
    ];
  return new contracts_1.Contract(
    address,
    [
...
class BurnRequest extends Request_1.Request {
  ...
  supportsERC20Permit() {
    return !(
      ...
      isWBTC(this.asset) ||
      (this.getChainId() === 1 &&
        (0, address_1.getAddress)(common_1.FIXTURES.ETHEREUM.USDT) ===
          (0, address_1.getAddress)(this.asset))
    );
  }
  async needsApproval() {
    const contract = new contracts_1.Contract(
  ...
  async approve(signer, amount) {
    if (!amount) amount = constants_1.MaxUint256;
    const contract = new contracts_1.Contract(
      this.asset,
  ...
  getHandlerForDestinationChain() {
    return (0, common_1.isZcashAddress)(this.destination)
      ? ZECHandler_1.ZECHandler
      : BTCHandler_1.BTCHandler;
  }
  getNormalizedDestinationAddress() {
    if ((0, common_1.isZcashAddress)(this.destination))
      return Buffer.from(
        (0, bytes_1.hexlify)(this.destination).substr(2),
        "hex"
      ).toString("utf8"); // implement zcash encoding here
    ...
    else address = basex_1.Base58.encode(this.destination);
    return address;
  }
  ...
  async sign(signer) {
    ...
    try {
      ...
      return (this.signature = (0, bytes_1.joinSignature)(
        (0, bytes_1.splitSignature)(sig)
      ));
    } catch (e) {
      console.error(e);
  ...
  async fetchData() {
    if (
      ...
      this.nonce = String(
        await new contracts_1.Contract(
          this.contractAddress,
      ...
      (0, address_1.getAddress)(common_1.FIXTURES.AVALANCHE.USDC) ===
      (0, address_1.getAddress)(this.asset)
    ) {
      this.nonce = String(
        await new contracts_1.Contract(
      ...
      const token = new contracts_1.Contract(
        this.asset,
        [
          "function nonces(address) view returns (uint256)",
