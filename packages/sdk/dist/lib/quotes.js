...
exports.makeQuoter = exports.returnChainDetails = void 0;
...
const returnChainDetails = (CHAINID, _provider) => {
    const provider = (chain) => _provider ||
...
exports.returnChainDetails = returnChainDetails;
function makeQuoter(CHAIN = "1", provider) {
    const chain = (0, exports.returnChainDetails)(CHAIN, provider);
    const renCrv = new ethers.Contract(fixtures[chain.name]["Curve_Ren"], [
    ...
    const quoter = new ethers.Contract("0xb27308f9F90D607463bb33eA1BeBb41C27CE5AB6", [
    ...
    const getAVAXQuote = async (direction, amount) => {
        const WBTC = new JOE.Token(JOE.ChainId.AVALANCHE, fixtures.AVALANCHE.WBTC, 8);
        ...
        if (direction) {
            ...
            const trade = new JOE.Trade(route, new JOE.TokenAmount(WBTC, wbtcAmount), JOE.TradeType.EXACT_INPUT);
            ...
            const trade = new JOE.Trade(route, new JOE.TokenAmount(JOE.WAVAX[JOE.ChainId.AVALANCHE], amount), JOE.TradeType.EXACT_INPUT);
            return await getWbtcQuote(false, ethers.utils.parseUnits(trade.outputAmount.toExact(), 8));
        }
    };
    // direction = true ? usdc -> renbtc
    const getUsdcQuoteAVAX = async (direction, amount) => {
        //amount = renBTC amount
        const aTricrypto = new ethers.Contract("0xB755B949C126C04e0348DD881a5cF55d424742B2", ["function get_dy(uint256, uint256, uint256) view returns (uint256)"], chain.provider);
        const crvUSD = new ethers.Contract("0x7f90122BF0700F9E7e1F688fe926940E8839F353", [
    ...
    const fromUSDC = async (amount) => {
        if (chain.name === "AVALANCHE") {
            ...
            let output = null;
            try {
                output = await quoter.quoteExactInput(ethers.utils.solidityPack(["address", "uint24", "address", "uint24", "address"], [
    ...
    const toUSDC = async (amount) => {
        try {
            if (chain.name === "AVALANCHE") {
                ...
                return await quoter.quoteExactInput(ethers.utils.solidityPack(["address", "uint24", "address", "uint24", "address"], [
                    fixtures[chain.name].WBTC,
                    500,
                    fixtures[chain.name].wETH,
                    500,
    ...
    const ETHtoRenBTC = async (amount) => {
        if (chain.name === "AVALANCHE") {
            ...
            const output = await quoter.quoteExactInput(ethers.utils.solidityPack(["address", "uint24", "address"].concat(chain.name === "MATIC" ? ["uint24", "address"] : []), path), amount);
            const result = await getWbtcQuote(false, output);
            return result;
        }
    };
    ...
    const wNativeToUSDC = async (amount) => {
        return await quoter.quoteExactInput(ethers.utils.solidityPack(["address", "uint24", "address"], [fixtures[chain.name].wNative, 500, fixtures[chain.name].USDC]), amount);
    };
    const renBTCToETH = async (amount) => {
        if (chain.name === "AVALANCHE") {
            ...
            path.splice(2, chain.name !== "MATIC" ? 2 : 0);
            const wbtcOut = await getWbtcQuote(true, amount);
            const quote = await quoter.quoteExactInput(ethers.utils.solidityPack(["address", "uint24", "address"].concat(chain.name === "MATIC" ? ["uint24", "address"] : []), path), wbtcOut);
            return quote;
        }
    };
    return {
