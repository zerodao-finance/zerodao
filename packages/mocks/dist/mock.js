...
const enableGlobalMockRuntime = () => {
    ...
    core_1.ZeroUser.prototype.publishTransferRequest = async function (transferRequest) {
        setTimeout(() => {
            (async () => {
                try {
                    Promise.all(keepers.map(async (v) => v._txDispatcher && v._txDispatcher(transferRequest))).catch(console.error);
                }
                catch (e) {
                    console.error(e);
                }
    ...
    core_1.ZeroUser.prototype.publishMetaRequest = async function (metaRequest) {
        try {
            await Promise.all(keepers.map(async (v) => {
                if (v._txDispatcher)
    ...
    zero_1.UnderwriterTransferRequest.prototype.submitToRenVM = async function () {
